    ...
    def __init__(self):
        ...
        def ent():
            if len(a.get()) ==0:
                ...
                d = messagebox.askyesno("Confirm", "Voullez vous vraiment supprimer le livre?")
                if d:
                    try:
                        ...
                        self.myCursor.close()
                        self.conn.close()
                        messagebox.showinfo("Confirm","Le Livre a été supprimer")
                        a.set("")
                    except:
                        messagebox.showerror("Error","Something goes wrong")
        Label(self, text = "Livre Id: ",bg='#e5e5e5',fg='black',font=('Courier new', 15, 'bold')).place(x = 5,y = 40)
